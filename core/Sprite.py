...
class Sprite(Animation):
    ...
    def display(self, surface: pygame.Surface, alpha: float = 100, center: str = 'NA', adjust: str = 'NA', frame: int = 0, bright: float = 100) -> None:
        # 从
        self.this = int(frame)
        t_m = self.this % self.mouth.length
        t_e = (self.this//self.mouth.length)%self.eye.length
        t_f = self.this//(self.mouth.length*self.eye.length)
        # face层
    ...
    def get_tick(self, duration: int, audio:Audio=None, delay:int=0, framerate:int=30) -> np.ndarray:
        """生成动画的时间轴
            - audio：Audio对象，音频
            - delay：整数，音频延后开始的时间
            - framerate：整数，项目的帧率
    ...
    def configure(self, key: str, value, index: int = 0):
        ...
        elif key in ['pos','scale']:
            # 如果是scale，self的尺寸在super已经更新，eye和mouth的尺寸需要额外更新
            self.eye.configure(key=key,value=value)
            self.mouth.configure(key=key,value=value)
