...
class Mp4Frame(AppFrame):
    ...
    def createWidgets(self):
        ...
        tk.Entry(filepath_v, textvariable=self.v["media_define"]).place(x=80,y=5+3,width=430,height=25)
        tk.Entry(filepath_v, textvariable=self.v["characor_table"],state=tk.DISABLED).place(x=80,y=50+3,width=430,height=25)
        tk.Entry(filepath_v, textvariable=self.v["timeline_file"]).place(x=80,y=95+3,width=430,height=25)
        tk.Entry(filepath_v, textvariable=self.v["output_path"]).place(x=80,y=140+3,width=430,height=25)
        tk.Button(filepath_v, command=lambda:self.call_browse_file(self.v["media_define"]),text="浏览").place(x=520,y=5,width=70,height=30)
        tk.Button(filepath_v, command=lambda:self.call_browse_file(self.v["characor_table"]),text="浏览",state=tk.DISABLED).place(x=520,y=50,width=70,height=30)
        tk.Button(filepath_v, command=lambda:self.call_browse_file(self.v["timeline_file"]),text="浏览").place(x=520,y=95,width=70,height=30)
        tk.Button(filepath_v, command=lambda:self.call_browse_file(self.v["output_path"],'path'),text="浏览").place(x=520,y=140,width=70,height=30)
        ...
        tk.Entry(optional_v,textvariable=self.v["project_F"]).place(x=380,y=5,width=70,height=25)
        tk.Entry(optional_v,textvariable=self.v["project_Z"]).place(x=80,y=50,width=220,height=25)
        tk.Entry(optional_v,textvariable=self.v["project_Q"]).place(x=380,y=50,width=70,height=25)
        ...
        tk.Button(self, command=self.run_command_mp4,text="开始",font=self.v["big_text"]).place(x=260,y=435,width=100,height=50)
